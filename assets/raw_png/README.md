...
Нужны **три иконки одной визуальной системы**: одна «линия» арта, как три варианта одного набора HUD, а не три случайных картинки. **Исключения для базовых Q, W, E:** на ровном `#fc03f8` без печати — **`rune_q`** ледяной осколок; **`rune_w`** цепная молния (ломаная линия между **маленькими кружками**-целями); **`rune_e`** метеор (читаемость в 64×64).
...
- **Центр стихии:** **Q** — **простой ледяной осколок** без обода; **W** — **цепная молния**: зигзаг между маленькими кругами-целями (жёлто-белый разряд); **E** — **метеор** без обода; альтернативный бриф для общей печати — см. **`invoke.png`** и [`palette.js`](../visuals/palette.js).
...
| `rune_w` | `This icon: chain lightning only — sharp jagged bolt jumping between several small plain circle targets, yellow-white core, no frame, no magic rings, no extra VFX.` |

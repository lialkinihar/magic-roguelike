...
**`duo_ww` (WW)** — **Chain Storm**: усиленная **цепная молния** как у **`rune_w`**, но плотнее разветвления, сильнее «шторм»; узлы смещены по высоте, ломаная траектория, **не** шаровая молния (это WE). В репозитории могут лежать варианты мастеров **`duo_ww_v1.png` … `duo_ww_v5.png`**; в **`import-manifest.json`** для одного `skillId` **`duo_ww`** укажи **один** файл — тот, который пойдёт в игру после `npm run icons:import-raw`. Сгенерированные отличия: **v1** — диагональная цепь, много узлов; **v2** — широкая дуга, длинные сегменты; **v3** — доминирующая S-зигзаг; **v4** — разветвление «Y» с сходом; **v5** — компактное ядро шторма с короткими дугами наружу.

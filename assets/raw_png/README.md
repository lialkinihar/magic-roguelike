...
| `combo_www` | `This icon: LIGHTNING ANCHOR — WWW: central ANCHOR readable, electric-plasma charged. CRITICAL: the anchor's ring, shank, and crossbar must visibly be made of or wrapped in a literal LIGHTNING CHAIN — many short ZIGZAG bolt segments linked end-to-end like chain links, NOT smooth metal-only lines. Every outgoing tether is the SAME: a CONTINUOUS CHAIN of linked lightning zigzag segments (repeating forks) radiating in multiple directions — NOT straight neon rays, NOT one thick beam without links. Yellow-white cores, cyan-white glow per segment. Flat chroma background pure #00ff00 ONLY — NO green in subject. Exactly 500x500 px. No text, no watermark. (Import: chromaHex "00ff00".)` |
...
**`combo_wqw` (WQW)** — **Solar Beam**: **широкий** плазменный луч, **внутри** — **лава / плазма** (оранжево-жёлтые завихрения, яркое ядро, искры), читается **полёт вперёд** (шлейф сзади); **немного** коротких молний по краям; фон **`#fc03f8`**.
...
**`combo_www` (WWW)** — **Lightning Anchor**: **якорь** и **все цепи** — буквально **звенья из молнии** (короткие зигзаги, связанные в цепь), в том числе **вдоль кольца и штока**; от якоря такие же **молниеносные цепи** уходят **в разные стороны**; фон **`#00ff00`**, **`chromaHex` `00ff00`** в манифесте.

...
Нужны **три иконки одной визуальной системы**: одна «линия» арта, как три варианта одного набора HUD, а не три случайных картинки. **Исключения для базовых Q и E (как у простого метеора):** **`rune_q`** — только **ледяной осколок** на ровном `#fc03f8`, без печати и без лишних эффектов; **`rune_e`** — только **метеор** на том же фоне (читаемость в 64×64).
...
- **Центр стихии:** **Q** — в этом репозитории **простой ледяной осколок** без обода (см. исключение выше), иначе лёд / кристалл в общей печати; **W** — молния (жёлтый, белый); **E** — **метеор** без обода; для альтернативного брифа E — пламя как у **`invoke.png`** и [`palette.js`](../visuals/palette.js).

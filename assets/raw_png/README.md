...
| `combo_qwq` | `This icon: ABSOLUTE ZERO — the GROUND in frame is FULLY FROZEN: cracked permafrost, hoarfrost, icy glaze, cyan-white-blue. A SMALL CLASSIC THERMOMETER above the ice with column at the BOTTOM / zero extreme (coldest), glass tube, mercury or alcohol at minimum — scale as ticks only, NO readable words. NO characters, NO fire, NO lightning, NO magic circles. Flat chroma #fc03f8 only. Exactly 500x500 px. No text, no watermark.` |
...
**`combo_qqq` (QQQ)** — **Cold Embrace**: **ледяной кокон** из **нескольких крупных ледяных чешуек** (накладные пластины, объятие); **внутри едва видна тень** фигуры сквозь полупрозрачный лёд; читается как **защита / оболочка**, не решётка и не тюрьма; фон **`#fc03f8`**.
...
**`combo_qwq` (QWQ)** — **Absolute Zero**: иконка — **целиком промерзшая земля**; **термометр** над льдом, столбик **у нуля**; фон **`#fc03f8`**.

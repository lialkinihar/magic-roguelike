...
Нужны **три иконки одной визуальной системы**: одна «линия» арта, как три варианта одного набора HUD, а не три случайных картинки. **Исключение для `rune_e` (E):** базовый навык «метеор» — **без рамки и без общей печати** с Q/W: только простой метеор на ровном `#fc03f8` (читаемость в 64×64), без лишних эффектов.
...
- **Центр стихии:** **Q** — лёд / кристалл / снежинка (циан, белый); **W** — молния (жёлтый, белый); **E** — в этом репозитории **метеор** без обода (см. исключение выше); для альтернативного брифа — пламя как у **`invoke.png`** и [`palette.js`](../visuals/palette.js).

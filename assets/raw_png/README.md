...
Нужны **три иконки одной визуальной системы**: одна «линия» арта, как три варианта одного набора HUD, а не три случайных картинки. **Исключения для базовых Q, W, E:** на ровном `#fc03f8` без печати — **`rune_q`** ледяной осколок; **`rune_w`** цепная молния: ломаная линия между **узлами**, где каждый узел — **сплетение молнии вокруг точки**, без видимого круга/диска; **`rune_e`** метеор (читаемость в 64×64).
...
- **Центр стихии:** **Q** — **простой ледяной осколок** без обода; **W** — **цепная молния**: зигзаг между узлами; в каждом узле молния **опутывает** «цель» так, что **самого круга не видно** (только клубок разрядов); **E** — **метеор** без обода; альтернативный бриф для общей печати — см. **`invoke.png`** и [`palette.js`](../visuals/palette.js).
...
| `rune_w` | `This icon: chain lightning — jagged bolt between nodes; each node is a tight ball of branching lightning wrapping the hit point so NO visible circle or solid disk shows through; yellow-white, no frame, no extra VFX.` |

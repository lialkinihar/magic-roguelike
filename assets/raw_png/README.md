...
| `combo_wwq` | `This icon: ENERGY SHIELD — WWQ: a stylized WIZARD silhouette (hood, staff) at center, READABLE. A thin SEMI-TRANSPARENT energy barrier WRAPS the whole body like a SECOND SKIN — the shield CONFORMS tightly to hood, shoulders, robe, and staff outline — NOT a large detached bubble, NOT a flat shield plate floating in front only, NOT a ring far from the figure. Cyan-white to violet-white electric sheen with subtle contour-hugging edge glow. Mage visible inside the skin-tight shell. NO giant lightning bolt as sole subject. Flat chroma #fc03f8 only. Exactly 500x500 px. No text, no watermark.` |
...
**`combo_wwq` (WWQ)** — **Energy Shield**: **щит** как **вторая кожа** — **плотно облегает** силуэт **мага** (капюшон, посох), **не** отдельный шар и **не** только пластина впереди; фон **`#fc03f8`**.
